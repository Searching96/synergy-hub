...
class TwoFactorAuthIntegrationTest {
    ...
    void complete2FASetupFlow_EndToEnd_ShouldWorkCorrectly() throws Exception {
        // Step 1: Setup 2FA
        TwoFactorSetupResponse setupResponse = twoFactorAuthService.setupTwoFactor(testUser);
        ...
        boolean verifyResult = twoFactorAuthService.verifyAndEnableTwoFactor(testUser, validCode);
        ...
        Thread.sleep(1000); // Wait 1 second to potentially get a new time window
        ...
        boolean loginVerifyResult = twoFactorAuthService.verifyCode(updatedUser, loginCode);
        assertThat(loginVerifyResult).isTrue();

        // Step 5: Test backup code
        String backupCode = setupResponse.getBackupCodes().get(0);
        boolean backupResult = twoFactorAuthService.verifyCode(updatedUser, backupCode);
        ...
        boolean sameBackupResult = twoFactorAuthService.verifyCode(updatedUser, backupCode);
        assertThat(sameBackupResult).isFalse();

        // Step 7: Disable 2FA
        twoFactorAuthService.disableTwoFactor(updatedUser, "password123");
    ...
    void verifyAndEnable_WithInvalidCode_ShouldNotEnable2FA() throws Exception {
        ...
        boolean result = twoFactorAuthService.verifyAndEnableTwoFactor(testUser, "000000");
    ...
    void verifyCode_WithAllBackupCodesUsed_ShouldOnlyAcceptTOTP() throws Exception {
        ...
        twoFactorAuthService.verifyAndEnableTwoFactor(testUser, validCode);

        User updatedUser = userRepository.findById(testUser.getId()).orElseThrow();

        // Use all backup codes
        for (String backupCode : setupResponse.getBackupCodes()) {
            boolean result = twoFactorAuthService.verifyCode(updatedUser, backupCode);
        ...
        boolean totpResult = twoFactorAuthService.verifyCode(updatedUser, totpCode);
